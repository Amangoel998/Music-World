...
const {
  ...
  ratingvalidators,
} = require("../middleware/updatevalidators");
...
router.post(
  ...
  [
    ...
    check("artists", "Atleast one Artist is required").not().isEmpty(),
  ],
  async (req, res) => {
    try {
      ...
      const artists = req.body.artists.filter((el) => (el.length > 2 ? true : false));
...
router.put("/", auth, songupdatevalidators, async (req, res) => {
  try {
    ...
    if (req.body.releasedate)
      updatesong.song_releasedate = req.body.releasedate;
...
router.put("/:rating", auth, ratingvalidators, async (req, res) => {
  const payload = {
